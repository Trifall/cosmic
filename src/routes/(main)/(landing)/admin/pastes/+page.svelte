<script lang="ts">
	...
	let allPastes = $derived<SinglePasteData[]>(data.pastes || []);
	...
	let pasteToDelete = $state<SinglePasteData | null>(null);

	// change owner modal state
	let isChangeOwnerModalOpen = $state(false);
	let selectedPaste = $state<SinglePasteData | null>(null);
	...
	const columns: ColumnDef<SinglePasteData>[] = [
		...
		},
		{
			...
			cell: ({ row }) => {
				const paste = row.original;
				const ownerSnippet = createRawSnippet<[string | null | undefined]>((getOwner) => {
					const owner = getOwner();
					return {
						render: () =>
							`<div class="truncate font-medium text-primary" title="${owner || ''}">${owner || 'Unknown'}</div>`,
					};
				});
				return renderSnippet(ownerSnippet, paste.ownerUsername);
			},
			meta: {
				headerClass: 'w-36',
				cellClass: 'w-36',
	...
	const openChangeOwnerModal = (paste: SinglePasteData) => {
		selectedPaste = paste;
...
<div class="container mx-auto px-4">
	<div class="mb-6 flex items-center justify-between">
		<h1 class="text-3xl font-bold">Admin - All Pastes</h1>
	</div>
	...
	<div class="w-full max-w-full">
		...
		<div class="mb-4 flex w-full max-w-md gap-2">
			<input
				type="text"
				placeholder="Search pastes (title, content, language, owner)..."
		...
		<DataTable
			...
			onOptimisticDelete={(deletedId) => {
				allPastes = allPastes.filter((paste) => paste.id !== deletedId);
			}}
		>
			{#snippet emptyState()}
				{#if data.search}
...
>
	<AlertDialogContent>
		<AlertDialogHeader>
			...
			<AlertDialogDescription>
				You are about to delete paste "{pasteToDelete?.title || `Paste ${pasteToDelete?.id}`}" owned
				by "{pasteToDelete?.ownerUsername}". This action cannot be undone.
			</AlertDialogDescription>
		</AlertDialogHeader>
		<AlertDialogFooter>
